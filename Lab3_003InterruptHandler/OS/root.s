...
reset_handler:
    ...
    msr cpsr_c, #0xD2    @ Cambia a modo IRQ
    ldr sp, =_stack_top  @ Configura su pila
    msr cpsr_c, #0xDF    @ Vuelve a modo System
...
irq_handler:
    push {r0-r12, lr} //guarda una copia de los registros
    bl timer_irq_handler // Salta a la ejecucion de la funcion timer_irq_handler que se escribio en C
    pop {r0-r12, lr} // Restaura los valores orignales que se habian gaurdado
    subs pc, lr, #4 // Hace que el procesador vuelva a la instruccion que se quedo a medias
...
.globl enable_irq
enable_irq:
    mrs r0, CPSR //Se copia ya que no se puede modificar directamente CPSR 
    bic r0, r0, #0x80 //Realiza Bit Clear
    msr CPSR_c, r0 // se escribe el valor modificado en CPSR
    bx lr // Salta a la direccion guardada en el Link Register
